...
$cardNum = $_POST['card'];
$cardHold = $_POST['card holder name'];
...
if (array_key_exists("SubmitThis", $_POST)) {
	...
	$required = array('fName', 'lName', 'address1', 'payment', 'email');
	$expected = array('fName', 'lName', 'address1', 'payment', 'email');
	$missing = array();
    ...
    if (!empty($email)) {
      ...
      $success="<h1>We've Received Your Reservation!</h1>";
		...
		$output="<p>Thank you <span class='resEmphasis'>$fName</span>, we'll have your order ready for pickup of <span class='resEmphasis'>$partySize</span> at <span class='resEmphasis'>$time</span> on <span class='resEmphasis'>$date</span>. </p> <p>Your code is:</p><span id='resCode'>$code</span> <p>Show this code to the host to grab your order.</p>";
    $to="kathryn.kerr@mavs.uta.edu, $receiver"; // change this to your own email address
    $subject="Bella Vita Order Confirmation";
    $header="From: bellavita@mysite.com";
    $message="Thank you $fName $lName, we recieved your order details on $date.

    Your confirmation number is $code, we'll have your order ready at $time.

    Show the confirmation code to the host to pick up your order. We can't wait to see you soon!";
